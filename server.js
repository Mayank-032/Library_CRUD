...
app.post('/Task3', async (req, res) => {
    ...
    if(req.body.issue !== undefined){
        if(await db.collection('books')
        .count({book_name: new RegExp(req.body.BookName3, 'i')}) > 0){
            
            if(await db.collection('Transaction')
            .count({$and:[{book_name: new RegExp(req.body.BookName3, 'i')}, {person_name: new RegExp(req.body.PersonName, 'i')}]}) === 0){
                
                await db.collection('Transaction')
                .insertOne({book_name: req.body.BookName3, person_name: req.body.PersonName, issueDate: new Date()})
        ...
        if(await db.collection('Transaction')
        .countDocuments({$and:[{book_name: req.body.BookName3}, {person_name: req.body.PersonName}]}) > 0){
            
            await db.collection('Transaction')
            .findOneAndUpdate({book_name: req.body.BookName3, person_name: req.body.PersonName}, {$set: {returnDate: new Date()}})
        
            let rentPerDay = await db.collection('books')
            .findOne({book_name: new RegExp(req.body.BookName3, 'i')}, {person_name: new RegExp(req.body.PersonName, 'i')})
            
            let dates = await db.collection('Transaction')
            .findOne({book_name: req.body.BookName3, person_name: req.body.PersonName})
            ...
            res.send("Book Returned Successfully!!! Total Rent Calculated: " + ((TotalDays+1)*rpd));
        }else{
